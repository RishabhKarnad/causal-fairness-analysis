...
x = PrettyTable()
x.field_names = ["Model", "SCM", "Data",
                 "Explainability Method", 'z', 'x', 'w']
...
for scm_type in ['fair', 'unfair']:
    for data_type in ['linear', 'nonlinear']:
        ...
        # os.listdir(os.path.join(models_path,scm_type,data_type)):
        for model_name in ["LR", 'mlp', 'dt']:
            model_path = os.path.join(
                models_path, scm_type, data_type, model_name + ".joblib")
            ...
            x.add_row([model_name.split(".")[0], scm_type, data_type, 'LIME', round(max_feature_contribution_lime['z'], 4), round(
                max_feature_contribution_lime['x'], 4), round(max_feature_contribution_lime['w'], 4)])
            x.add_row([model_name.split(".")[0], scm_type, data_type, 'SHAP', round(max_feature_contribution_shap['z'], 4), round(
                max_feature_contribution_shap['x'], 4), round(max_feature_contribution_shap['w'], 4)])
print(x)
