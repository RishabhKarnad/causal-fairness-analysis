...
class FairNonlinearGaussianSCM(StandardModelSCM):
    ...
    def p_y_zxw(self, z, x, w):
        ...
        # print(a)
        # stats.bernoulli(1/(1+np.exp(0.01*(-2*z+1))))
        return 0 if a < 0.425 else 1
...
class UnfairNonlinearGaussianSCM(StandardModelSCM):
    ...
    def p_x_z(self, z):
        mu_x_z = (2*z**3 - 14*z + 10) / 100000000
        return stats.norm(mu_x_z, 0.2)

    def p_w_zx(self, z, x):
        mu_w_zx = (5*x**2 - 12) / 1000000000
        return stats.norm(mu_w_zx, 0.2)

    def p_y_zxw(self, z, x, w):
        a = 1/(1+np.exp(0.1*(-2*z+13*x-2*w+25)))
        # print(a)
        # stats.bernoulli(1/(1+np.exp(0.001*(-10*z+0.00000013*x-0.000000002*w+1))))
        return 0 if a < 0.625 else 1
